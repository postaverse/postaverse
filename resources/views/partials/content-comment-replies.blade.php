@props(['reply', 'replyingTo', 'contentType'])

<div
    class="bg-gray-800/30 backdrop-blur-sm border border-white/5 rounded-lg p-3 hover:border-white/10 transition-all mb-3">
    <!-- Reply User Info -->
    <div class="flex justify-between mb-3">
        <div class="flex space-x-3">
            <img src="{{ $reply->user->profile_photo_url }}"
                alt="{{ $reply->user->name ?: $reply->user->handle }}'s profile photo" class="w-8 h-8 rounded-full">
            <div>
        ...
        <div class="flex items-center space-x-3">
            ...
            @if (auth()->user() &&
                    ($reply->user_id == auth()->user()->id ||
                        ($contentType === 'blog' || $contentType === 'blogs' ? $reply->blog->user_id : $reply->post->user_id) ==
                            auth()->user()->id ||
    ...
    @if (auth()->user() && $replyingTo === $reply->id)
        <div class="mt-4 pl-4 border-l-2 border-indigo-500/30">
            <form wire:submit.prevent="submitReply" class="w-full">
                <div class="mb-3">
                    <textarea id="replyContent-{{ $reply->id }}"
