        ...
        <div class="flex items-center space-x-3">
            ...
            @if (auth()->user() && 
                ($reply->user_id == auth()->user()->id || 
                 (($contentType === 'blog' || $contentType === 'blogs') ? $reply->blog->user_id : $reply->post->user_id) == auth()->user()->id || 
    ...
    @if ($reply->replies && $reply->replies->count() > 0)
        <div class="mt-4 pl-4 border-l border-indigo-500/20">
            @foreach ($reply->replies as $nestedReply)
                @include('partials.content-comment-replies', ['reply' => $nestedReply, 'replyingTo' => $replyingTo, 'contentType' => $contentType])
            @endforeach
        </div>
    @endif
</div>
