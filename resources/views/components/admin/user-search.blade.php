...
<div>
    ...
    <div class="relative">
        <input type="text" wire:model.debounce.300ms="{{ $model }}"
            wire:keyup="{{ $searchMethod }}"
    ...
    @if (count($searchResults) > 0)
        <div class="mt-3 bg-gray-800/70 border border-gray-600 rounded-lg shadow-lg max-h-80 overflow-y-auto">
            <div class="p-2 bg-gray-700/50 border-b border-gray-600">
                <p class="text-sm text-gray-300">{{ count($searchResults) }} user(s) found</p>
            </div>
            @foreach ($searchResults as $user)
                <x-admin.user-search-result 
                    :user="$user" 
                    :selectMethod="$selectMethod" />
            @endforeach
        </div>
    @elseif(strlen($this->{$model} ?? '') >= 2)
        <div class="mt-3 bg-gray-800/70 border border-gray-600 rounded-lg p-4 text-center">
            <div class="text-gray-400">
                ...
                <p>No users found matching "{{ $this->{$model} }}"</p>
            </div>
        </div>
    @endif
</div>
