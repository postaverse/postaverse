...
<div x-data="{ show: @entangle($attributes->wire('model')) }" x-on:close.stop="show = false" x-on:keydown.escape.window="show = false" x-show="show"
    id="{{ $id }}" class=" fixed inset-0 overflow-y-auto px-4 py-6 sm:px-0"
    ...
    <div x-show="show"
        class="mb-6 bg-gray-800/10 backdrop-blur-sm border border-white/20 rounded-lg overflow-hidden shadow-sm transform transition-all sm:w-full {{ $maxWidth }} sm:mx-auto"
