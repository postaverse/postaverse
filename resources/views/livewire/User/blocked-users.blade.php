        ...
        <div class="col-span-6 sm:col-span-4">
            <div class="flex">
                <div class="grow mr-2">
                    ...
                    <x-input id="username" type="text" wire:model="username"
                        class="block w-full mt-1 bg-gray-800/40 backdrop-blur-sm border border-white/10 text-white focus:border-indigo-500/50 focus:ring-2 focus:ring-indigo-500/20 focus:outline-none transition-all"
                        name="username" autocomplete="off" placeholder="Enter a username" />
                </div>
                <div class="flex items-end">
                    <x-button type="button" wire:click="addUser"
        ...
        <div class="col-span-6 sm:col-span-4 mt-4">
            ...
            @if (count($blockedUsersList) > 0)
                <ul class="mt-2 border border-white/10 rounded-lg divide-y divide-white/10 overflow-hidden">
                    @foreach ($blockedUsersList as $user)
                        <li
                            class="px-4 py-3 flex items-center justify-between bg-gray-800/40 backdrop-blur-sm hover:bg-gray-800/60 transition-colors">
                            <span class="text-white font-medium">{{ '@' . $user['handle'] }}</span>
                            <button type="button" wire:click="removeUser('{{ $user['id'] }}')"
                                class="text-red-400 hover:text-red-300 focus:outline-none transition-colors duration-200 p-1.5 hover:bg-red-500/10 rounded-full">
            ...
            @else
                <div class="p-4 bg-gray-800/40 backdrop-blur-sm rounded-lg border border-white/10 text-center">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-8 w-8 mx-auto text-gray-500 mb-2" fill="none"
