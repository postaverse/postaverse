        ...
        <div class="p-6">
                    ...
                    @foreach($followers as $follower)
                        <div class="flex items-center justify-between p-4 bg-zinc-800 rounded-lg">
                            <div class="flex items-center space-x-3">
                                {{-- Avatar --}}
                                <a href="{{ route('user.profile', $follower->username) }}" wire:navigate>
                                    @if($follower->avatar)
                                        <img class="h-12 w-12 rounded-full object-cover" src="{{ Storage::url($follower->avatar) }}" alt="{{ $follower->display_name }}">
                                    @else
                                        <div class="h-12 w-12 rounded-full bg-zinc-700 flex items-center justify-center">
                                            <span class="text-zinc-100 font-medium">{{ substr($follower->username, 0, 1) }}</span>
                                        </div>
                                    @endif
                                </a>
                                    ...
                                    <div class="flex items-center space-x-2">
                                        ...
                                        @if($follower->admin_level > 0)
                                            <span class="bg-purple-100 text-purple-800 dark:bg-purple-900 dark:text-purple-200 text-xs font-medium px-2.5 py-0.5 rounded">
                                                @if($follower->admin_level === 1) Super Admin
                                                @elseif($follower->admin_level === 2) Admin
                                                @else Moderator @endif
                                            </span>
                                        @endif
                                    </div>
                                    <p class="text-zinc-400 text-sm">{{ "@" . $follower->username }}</p>
                                    @if($follower->bio)
                                        <p class="text-zinc-300 text-sm mt-1">{{ Str::limit($follower->bio, 60) }}</p>
                                    @endif
                                </div>
                            </div>
                            ...
                            @auth
                                @if($follower->id !== auth()->id())
                                    @if(auth()->user()->following()->where('following_id', $follower->id)->exists())
                                        <button wire:click="toggleFollow({{ $follower->id }})" 
            ...
            @else
                <div class="text-center py-8">
                    ...
                    <h3 class="text-lg font-medium text-zinc-300 mb-2">No followers yet</h3>
                    <p class="text-zinc-400">{{ $user->display_name }} doesn't have any followers yet.</p>
                </div>
            @endif
        </div>
    </div>
