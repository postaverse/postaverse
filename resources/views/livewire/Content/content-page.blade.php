        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 mb-8 w-full">
            <div
                ...
                <div class="flex items-center space-x-4 mb-6 pb-4 border-b border-white/10">
                    <img src="{{ $content->user->profile_photo_url }}"
                        alt="{{ $content->user->name ?: $content->user->handle }}'s profile photo"
                        class="w-12 h-12 rounded-full ring-2 ring-indigo-500/50">
                    <div>
                        <h2 class="text-lg font-bold text-white flex items-center">
                            <a href="{{ route('user-profile', $content->user->id) }}"
                ...
                <div class="flex items-center justify-between mt-6 pt-4 border-t border-white/10">
                    <!-- Like Count -->
                    <span class="text-white" id="likeCount-{{ $content->id }}">{{ $content->likes->count() }}
                        likes</span>

                    <!-- Delete Button -->
                    @if (auth()->user() && ($content->user_id == auth()->user()->id || auth()->user()->admin_rank >= 3))
                        <button
                            class="text-red-500 hover:text-red-400 font-semibold transition-colors flex items-center gap-1"
            ...
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 mb-8 w-full">
                ...
                <div
                    ...
                    <div class="flex items-center gap-6">
                        <!-- Like Button -->
                        <button wire:click="likeContent({{ $content->id }})"
                            class="text-white hover:scale-110 transition-transform likeButton"
                            data-content-id="{{ $content->id }}">
                            @if ($contentType === 'blog' || $contentType === 'blogs')
                                @if (!$content->likes->contains('user_id', auth()->id()))
                                    <img src="{{ asset('images/icons/like/unliked.png') }}" alt="Unlike"
                                        width="40" height="40"
                                        class="p-1 filter hover:brightness-125 transition-all">
                                @else
                        ...
                        <div class="flex -space-x-2 overflow-hidden">
                            @foreach ($content->likes->take(10) as $like)
                                <a href="{{ route('user-profile', $like->user->id) }}"
                                    class="relative hover:z-10 transition-all">
                            ...
                            @if ($content->likes->count() > 10)
                                <div
                                    class="flex items-center justify-center w-10 h-10 rounded-full bg-indigo-600 text-white border-2 border-gray-800 relative hover:z-10 font-semibold text-sm">
                        ...
                        <form wire:submit.prevent="submit" class="max-w-7xl w-full">
                            <div class="mb-4 max-w-7xl w-full">
                                ...
                                <textarea id="comment"
                                    ...
                                    wire:model="commentText" placeholder="Write your comment here..."></textarea>
                                @error('commentText')
                            ...
                            <x-button type="submit" class="bg-indigo-600 hover:bg-indigo-700 transition-colors">
                                <img src="{{ asset('images/icons/blastoff.png') }}" alt="Submit" width="30"
                                    height="30" class="p-1 pr-2">
                                {{ __('Post') }}
                            </x-button>
                            <script>
                ...
                <div class="space-y-4">
                    @if ($comments && $comments->count() > 0)
                        @foreach ($comments as $comment)
                            <div
                                class="bg-gray-800/40 backdrop-blur-sm border border-white/10 rounded-lg p-4 hover:border-white/20 transition-all">
                                <!-- Comment User Info -->
                                    ...
                                    <div class="flex items-center space-x-4">
                                        ...
                                        @if (auth()->user())
                                            <button wire:click="startReply({{ $comment->id }})"
                                                class="text-indigo-400 hover:text-indigo-300 transition-colors text-sm flex items-center gap-1">
                                ...
                                @if (isset($comment->has_profanity) && $comment->has_profanity)
                                    @if (auth()->user()->profanity_block_type == 'hide_clickable')
                                        <div>
                                            ...
                                            <div class="text-white prose prose-invert prose-sm mt-2"
                                                style="display:none;">
                                                {!! $comment->content !!}
                                            </div>
                                        </div>
                                    ...
                                    @endif
                                @else
                                    <div class="text-white prose prose-invert prose-sm mt-2">{!! $comment->content !!}
                                    </div>
                                @endif
                                        ...
                                        <form wire:submit.prevent="submitReply" class="w-full">
                                            <div class="mb-3">
                                                <textarea id="replyContent-{{ $comment->id }}"
                                                    ...
                                                    wire:model="replyContent" placeholder="Write your reply here..."></textarea>
                                                @error('replyContent')
                                                    <span
                                                        class="error text-red-500 text-xs mt-1">{{ $message }}</span>
                                                @enderror
                                            </div>
                                            <div class="flex space-x-2">
                                                <x-button type="submit"
                                                    class="bg-indigo-600 hover:bg-indigo-700 transition-colors py-1 px-3 text-sm">
                                                    {{ __('Post Reply') }}
                                                </x-button>
                                ...
                                @if ($comment->replies && $comment->replies->count() > 0)
                                    <div class="mt-4 pl-4 border-l border-indigo-500/20">
                                        @foreach ($comment->replies as $reply)
                                            @include('partials.content-comment-replies', [
                                                'reply' => $reply,
