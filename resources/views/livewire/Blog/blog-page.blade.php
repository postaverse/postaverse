            ...
            <div
                ...
                <div class="flex items-center space-x-4 mb-6 pb-4 border-b border-white/10">
                    <img src="{{ $blog->user->profile_photo_url }}"
                        alt="{{ $blog->user->name ?: $blog->user->handle }}'s profile photo"
                ...
                <div class="flex items-center justify-between mt-6 pt-4 border-t border-white/10">
                    <!-- Like Count -->
                    <span class="text-white" id="likeCount-{{ $blog->id }}">{{ $blog->likes->count() }}
                        likes</span>
            ...
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 mb-8 w-full">
                ...
                <div
                    ...
                    <div class="flex items-center gap-6">
                        <!-- Like Button -->
                        <button wire:click="likeBlog({{ $blog->id }})"
                            class="text-white hover:scale-110 transition-transform" id="likeButton">
                        ...
                        <form wire:submit.prevent="submit" class="max-w-7xl w-full">
                            <div class="mb-4 max-w-7xl w-full">
                                ...
                                <textarea id="comment"
                                    ...
                                    wire:model="content" placeholder="Write your comment here..."></textarea>
                                @error('content')
                                ...
                                @if ($comment->replies && $comment->replies->count() > 0)
                                    <div class="mt-4 pl-4 border-l border-indigo-500/20">
                                        @foreach ($comment->replies as $reply)
                                            @include('partials.blog-comment-replies', [
                                                'reply' => $reply,
